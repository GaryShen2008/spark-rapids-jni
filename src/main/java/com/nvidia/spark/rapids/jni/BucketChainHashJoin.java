...
public class BucketChainHashJoin {
    ...
    public static Table gather(Table table, ColumnView gatherMap) {
        return gather(table, gatherMap, OutOfBoundsPolicy.NULLIFY);
    }

    public static Table gather(Table table, ColumnView gatherMap, OutOfBoundsPolicy outOfBoundsPolicy) {
        boolean checkBounds = outOfBoundsPolicy == OutOfBoundsPolicy.NULLIFY;
        return new Table(gather(table.getNativeView(), gatherMap.getNativeView(),checkBounds));
    }
