...
//TEST_F(GenerateJoinTest, InnerJoinNoNulls)
//{
//  using Key = int32_t;
//  auto const right_size = static_cast<cudf::size_type>(1<<5);
//  auto const left_size = static_cast<cudf::size_type>(1<<5);
//  double const selectivity = 0.3;
//  int const multiplicity   = 1;
//  std::unique_ptr<cudf::column> right_key_column0 = cudf::make_numeric_column(cudf::data_type(cudf::type_to_id<Key>()), right_size);
//  std::unique_ptr<cudf::column> left_key_column0 = cudf::make_numeric_column(cudf::data_type(cudf::type_to_id<Key>()), left_size);
