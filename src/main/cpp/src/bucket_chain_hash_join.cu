...
//                            : cudf::nullable_join::NO;
  ...
  try{
      if(num_s > num_r){
        SortHashJoinV1 shj(left_input, right_input, 0, 16, circular_buffer_size, stream, mr);
        return shj.join();
      } else {
        SortHashJoinV1 shj(right_input, left_input, 0, 16, circular_buffer_size, stream, mr);
