...
//                            : cudf::nullable_join::NO;
  ...
  try{
      if(num_s > num_r){
        SortHashJoin shj(left_input, right_input, 0, 19, circular_buffer_size, stream, mr);
        auto result = shj.join();
        //std::cout << "partition time: " << shj.partition_time << std::endl;
        return result;
      } else {
        SortHashJoin shj(right_input, left_input, 0, 19, circular_buffer_size, stream, mr);
                              ...
                              rmm::device_async_resource_ref mr)
{
    ...
    SortHashGather shg(source_table, gather_map, n_match, num_rows, 0, 19, stream, mr);
